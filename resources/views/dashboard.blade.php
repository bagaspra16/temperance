...
<div class="container mx-auto px-2 sm:px-4 py-4 sm:py-8 text-gray-200 lg:px-8">
    ...
    <div class="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-6 sm:mb-8">
        <!-- Stats Cards -->
        <div class="bg-gradient-to-l from-gray-800 to-gray-900 rounded-lg sm:rounded-xl p-2 sm:p-3 md:p-4 hover:shadow-lg border border-gray-600 flex items-center justify-between transform hover:scale-105 transition-transform duration-300">
            <div>
                <h2 class="text-sm sm:text-lg font-semibold">Total Goals</h2>
                <p class="text-2xl sm:text-3xl font-bold">{{ $totalGoals }}</p>
            </div>
            <i class="fas fa-bullseye text-2xl sm:text-4xl text-pink-500"></i>
        </div>

        <div class="bg-gradient-to-r from-gray-800 to-gray-900 rounded-lg sm:rounded-xl p-2 sm:p-3 md:p-4 hover:shadow-lg border border-gray-600 flex items-center justify-between transform hover:scale-105 transition-transform duration-300">
            <div>
                <h2 class="text-sm sm:text-lg font-semibold">Completed Goals</h2>
                <p class="text-2xl sm:text-3xl font-bold">{{ $completedGoals }}</p>
            </div>
            <i class="fas fa-check-circle text-2xl sm:text-4xl text-green-500"></i>
        </div>

        <div class="bg-gradient-to-l from-gray-800 to-gray-900 rounded-lg sm:rounded-xl p-2 sm:p-3 md:p-4 hover:shadow-lg border border-gray-600 flex items-center justify-between transform hover:scale-105 transition-transform duration-300">
            <div>
                <h2 class="text-sm sm:text-lg font-semibold">Total Tasks</h2>
                <p class="text-2xl sm:text-3xl font-bold">{{ $totalTasks }}</p>
            </div>
            <i class="fas fa-tasks text-2xl sm:text-4xl text-blue-500"></i>
        </div>

        <div class="bg-gradient-to-r from-gray-800 to-gray-900 rounded-lg sm:rounded-xl p-2 sm:p-3 md:p-4 hover:shadow-lg border border-gray-600 flex items-center justify-between transform hover:scale-105 transition-transform duration-300">
            <div>
                <h2 class="text-sm sm:text-lg font-semibold">Completed Tasks</h2>
                <p class="text-2xl sm:text-3xl font-bold">{{ $completedTasks }}</p>
            </div>
            <i class="fas fa-clipboard-check text-2xl sm:text-4xl text-purple-500"></i>
        </div>
    </div>

    <!-- Calendar Section -->
    <div class="mt-6 sm:mt-8 mb-8 bg-gray-800/50 backdrop-blur-sm rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden border border-gray-600/50">
        <!-- Calendar Header -->
        <div class="bg-gradient-to-l from-gray-700/90 to-gray-800/90 backdrop-blur-sm p-3 sm:p-4 md:p-6 border-b border-gray-600/50">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-3 sm:space-y-0">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-2 sm:gap-4 mt-3 sm:mt-0">
                    <a href="{{ route('goals.calendar', ['year' => $currentDate->year, 'month' => $currentDate->month]) }}" class="bg-pink-600 hover:bg-pink-700 text-white font-semibold py-1.5 sm:py-2 px-3 sm:px-5 rounded-xl shadow-lg transform hover:scale-105 transition-all duration-300 text-center flex items-center text-xs sm:text-sm" onclick="showLoading('Memuat calendar view...', 'Mohon tunggu sebentar')">
        ...
        <div class="bg-gray-800/50 backdrop-blur-sm">
            ...
            <div class="grid grid-cols-7">
                @foreach($calendarDays as $day)
                    ...
                    <div class="calendar-day min-h-[60px] sm:min-h-[80px] md:min-h-[100px] lg:min-h-[120px] xl:min-h-[140px] border-r border-b border-gray-600/50 p-1 sm:p-2 md:p-3 {{ $day['isCurrentMonth'] ? 'bg-gray-800/50' : 'bg-gray-900/50' }} {{ $day['isToday'] ? 'bg-gradient-to-br from-pink-900/80 to-pink-800/80 border-pink-500/50' : '' }} {{ $day['isWeekend'] ? 'bg-gray-900/50' : '' }} hover:bg-gray-700/70 transition-all duration-300 group {{ ($taskEvents->count() > 0 || $goalEvents->count() > 0) ? 'cursor-pointer' : '' }}"
                        ...
                        <div class="flex justify-between items-start mb-1">
                            ...
                            @if($day['isToday'])
                                <span class="text-xs text-pink-300 font-bold bg-pink-900/50 px-1 py-0.5 rounded-full border border-pink-500/30">TODAY</span>
                            @endif
                        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8">
        <!-- Categories Section -->
        <div class="bg-gradient-to-l from-gray-800 to-gray-900 rounded-lg sm:rounded-xl p-2 sm:p-3 md:p-4 hover:shadow-lg border border-gray-600 flex flex-col h-[300px] sm:h-[350px] md:h-[400px]">
            <div class="flex justify-between items-center mb-4 flex-shrink-0">
                <h2 class="text-lg sm:text-xl font-semibold">Your Categories</h2>
                <a href="{{ route('categories.create') }}" class="bg-pink-500 hover:bg-pink-600 text-white px-2 sm:px-4 py-1.5 sm:py-2 rounded-md text-xs sm:text-sm" onclick="showLoading('Memuat halaman...', 'Mohon tunggu sebentar')">Add New</a>
            </div>

            @if($categories->count() > 0)
                <div class="flex-1 overflow-y-auto modal-scrollbar space-y-2 sm:space-y-3 pr-2">
                    @foreach($categories as $category)
                        <div class="flex items-center p-2 sm:p-3 border rounded-md border-gray-600 hover:bg-gray-700 transition-all duration-200 transform hover:scale-[1.02] hover:shadow-lg" style="border-left-color: {{ $category->color }}; border-left-width: 4px;">
                            <div class="flex-1">
                                <h3 class="font-medium text-white text-sm sm:text-base">{{ $category->name }}</h3>
                                <p class="text-xs sm:text-sm text-gray-400">{{ $category->goals->count() }} goals</p>
                            </div>
                            <a href="{{ route('categories.show', $category->id) }}" class="text-pink-500 hover:text-pink-400 p-1.5 sm:p-2 hover:bg-gray-600 rounded-lg transition-all duration-200" onclick="showLoading('Memuat detail...', 'Mohon tunggu sebentar')">
            ...
            @else
                <div class="flex-1 flex items-center justify-center">
                    <div class="text-center">
                        <div class="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-3 sm:mb-4 bg-gray-700 rounded-full flex items-center justify-center">
                            <i class="fas fa-folder-open text-xl sm:text-2xl text-gray-500"></i>
                        </div>
                        <p class="text-gray-500 text-xs sm:text-sm px-2">No categories yet. Create your first category to organize your goals.</p>
                    </div>
                </div>
            @endif
        </div>
        ...
        <div class="bg-gradient-to-r from-gray-800 to-gray-900 rounded-lg sm:rounded-xl p-2 sm:p-3 md:p-4 hover:shadow-lg border border-gray-600 flex flex-col h-[300px] sm:h-[350px] md:h-[400px]">
            <div class="flex justify-between items-center mb-4 flex-shrink-0">
                <h2 class="text-lg sm:text-xl font-semibold">Upcoming Goals</h2>
                <a href="{{ route('goals.create') }}" class="bg-pink-500 hover:bg-pink-600 text-white px-2 sm:px-4 py-1.5 sm:py-2 rounded-md text-xs sm:text-sm" onclick="showLoading('Memuat halaman...', 'Mohon tunggu sebentar')">Add New</a>
            </div>

            @if($upcomingGoals->count() > 0)
                <div class="flex-1 overflow-y-auto modal-scrollbar space-y-2 sm:space-y-3 pr-2">
                    @foreach($upcomingGoals as $goal)
                        <div class="p-2 sm:p-3 border rounded-md border-gray-600 hover:shadow-lg transition-all duration-200 transform hover:scale-[1.02] bg-gray-700/30">
                            ...
                            <div class="mb-2 sm:mb-3">
                                <div class="w-full bg-gray-700 rounded-full h-2 sm:h-2.5 overflow-hidden">
                            ...
                            <div class="flex justify-between items-center">
                                <a href="{{ route('goals.show', $goal->id) }}" class="text-xs sm:text-sm text-pink-500 hover:text-pink-400 flex items-center" onclick="showLoading('Memuat detail...', 'Mohon tunggu sebentar')">
                                    <i class="fas fa-eye mr-1"></i> View Details
                                </a>
                                @if($goal->category)
                                    <span class="text-xs text-gray-500 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full bg-gray-600/50" style="border-left-color: {{ $goal->category->color }}; border-left-width: 3px;">
                                        {{ $goal->category->name }}
                                    </span>
                                @endif
                            </div>
            ...
            @else
                <div class="flex-1 flex items-center justify-center">
                    <div class="text-center">
                        <div class="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-3 sm:mb-4 bg-gray-700 rounded-full flex items-center justify-center">
                            <i class="fas fa-bullseye text-xl sm:text-2xl text-gray-500"></i>
                        </div>
                        <p class="text-gray-500 text-xs sm:text-sm px-2">No upcoming goals. Create your first goal to start tracking your progress.</p>
                    </div>
                </div>
            @endif
        </div>
    ...
    <div class="mt-6 sm:mt-8 mb-8 bg-gradient-to-l from-gray-800 to-gray-900 rounded-lg sm:rounded-xl p-2 sm:p-3 md:p-4 hover:shadow-lg border border-gray-600">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-lg sm:text-xl font-semibold">Recent Tasks</h2>
            <a href="{{ route('tasks.create') }}" class="bg-pink-500 hover:bg-pink-600 text-white px-2 sm:px-4 py-1.5 sm:py-2 rounded-md text-xs sm:text-sm" onclick="showLoading('Memuat halaman...', 'Mohon tunggu sebentar')">Add New</a>
        </div>
        ...
        @else
            <p class="text-gray-500 text-sm">No tasks yet. Create your first task to start tracking your progress.</p>
        @endif
    </div>
