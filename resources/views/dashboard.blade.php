            ...
            @if($categories->count() > 0)
                <div class="space-y-3">
                    @foreach($categories as $category)
                        <div class="flex items-center p-3 border rounded-md border-gray-600" style="border-left-color: {{ $category->color }}; border-left-width: 4px;">
                            <div class="flex-1">
                                <h3 class="font-medium">{{ $category->name }}</h3>
                                <p class="text-sm text-gray-500">{{ $category->goals->count() }} goals</p>
                            </div>
                            <a href="{{ route('categories.show', $category->id) }}" class="text-pink-500 hover:text-600">View</a>
                        </div>
                    @endforeach
                </div>
            @else
    ...
    <div class="mt-8 bg-gray-800 rounded-lg shadow p-6">
        ...
        <div class="flex flex-col">
            <div class="-m-1.5 overflow-x-auto">
                <table class="min-w-full divide-y  divide-gray-800 rounded-full">
                    ...
                    <tbody class="bg-gray-700 divide-y divide-gray-600 rounded-lg">
                        @foreach($recentTasks as $task)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-400">{{ $task->title }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
